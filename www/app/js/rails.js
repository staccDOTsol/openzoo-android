(function (root, factory) {
  ...
  // Live Solana rails from a real 402 (2026-08-20). Ignore eip155 on Android.
  var PAYABLE = [
  ...
  // Underlying assets a user might hold instead of the NAV-wrapped twin.
  var UNDERLYING = [
    { symbol: "USDC", mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", twin: "yUSDCx" },
  ...
  function findPayableRow(accepts, symbol) {
    ...
    for (var j = 0; j < (accepts || []).length; j++) {
      var row = accepts[j];
      if (row.asset === spec.mint || rowSymbol(row) === symbol) return row;
    }
    return null;
  }
  ...
  function steerCopy(decision, helpText) {
    ...
    if (held.indexOf("TOKEN") !== -1) {
      extras.push("You hold TOKEN, but Solana settlement is in wTOKENx (the wrapped twin).");
    }
    if (held.indexOf("LEOS") !== -1) {
      extras.push("You hold LEOS, but Solana settlement is in wLEOSx (the wrapped twin).");
    }
    if (decision && decision.empty && helpText) extras.push(String(helpText));
    if (extras.length) body = body + "\n\n" + extras.join("\n\n");
    return {
      title: "Need a payable token",
      body: body,
      wrapUrl: wrap,
